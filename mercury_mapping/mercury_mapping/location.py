...
class Location:
    ...
    def add_pose(self, pose: Pose, update_position: bool, update_orientation: bool):
        ...
        if update_orientation:
            # Unit vector. Vision detects normal vector as Z axis so thats what we will rotate here
            unit = [0, 0, 1]
            ...
            # maybe someday roll and pitch will be nonzero. When that happens, do a projection with euler_angle[0]
            self.orientation["x"][self.orientation_location] = 0
            # maybe someday...
            self.orientation["y"][self.orientation_location] = 0
    ...
    def get_pose(self) -> PoseWithCovariance:
        ...
        if self.buffer_warm:
            for key in self.position.keys():
                trimmed[key] = remove_outliers(
                    self.position[key], self.quantile)
        else:
        ...
        if self.buffer_warm:
            for key in self.orientation.keys():
                trimmed[key] = remove_outliers(
                    self.orientation[key], self.quantile)
        else:
