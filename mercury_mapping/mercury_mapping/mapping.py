...
from mercury_msgs.srv import MappingTarget
from mercury_msgs.msg import MappingTargetInfo, LedCommand
...
class MappingNode(Node):
    ...
    def __init__(self):
        # Init the ROS Node
        super().__init__('mercury_mapping')
        ...
        self.target_object = ""
        self.lock_map = False
        self.offset = Location(Point(), Vector3(), int(self.get_parameter(
            "buffer_size").value), tuple(self.get_parameter("quantile").value))

        self.add_on_set_parameters_callback(self.param_callback)
        self.create_subscription(Detection3DArray, "detected_objects".format(
            self.get_namespace()), self.vision_callback, qos_profile_system_default)
        self.status_pub = self.create_publisher(
            MappingTargetInfo, "state/mapping", qos_profile_system_default)
        self.create_service(MappingTarget, "mapping_target",
                            self.target_callback)
        self.led_pulse_pub = self.create_publisher(
            LedCommand, "command/led", qos_profile_system_default)

        self.last_pub_time = Time()
        self.publish_pose()
        self.publish_timer = self.create_timer(
            0.125, self.update_oustanding_items)
    ...
    def create_location(self, object: str):
        ...
        xyz.x = float(self.get_parameter(
            'init_data.{}.pose.x'.format(object)).value)
        xyz.y = float(self.get_parameter(
            'init_data.{}.pose.y'.format(object)).value)
        xyz.z = float(self.get_parameter(
            'init_data.{}.pose.z'.format(object)).value)

        rpy.x = 0.0
        rpy.y = 0.0
        rpy.z = float(self.get_parameter(
            'init_data.{}.pose.yaw'.format(object)).value)

        self.objects[object]["location"] = Location(xyz, rpy, int(self.get_parameter(
            "buffer_size").value), tuple(self.get_parameter("quantile").value))
        ...
        (pose.orientation.w, pose.orientation.x, pose.orientation.y,
         pose.orientation.z) = euler2quat(rpy.x, rpy.y, rpy.z)
        self.objects[object]["init_pose"] = pose

    # Creates a publisher to publish PoseWithCovariance
    def add_publisher(self, object: str):
        self.objects[object]["publisher"] = self.create_publisher(
            PoseWithCovarianceStamped, "mapping/{}".format(object), qos_profile_system_default)
        ...
        for detection in detections.detections:
            ...
            if not result.hypothesis.class_id in self.objects.keys() \
                ...
                self.get_logger().warning(
                    f"Unknown class id {result.hypothesis.class_id}")
            ...
            if result.hypothesis.score < float(self.get_parameter("confidence_cutoff").value):
                self.get_logger().info(
                    f"Rejecting detection of {result.hypothesis.class_id} because confidence {result.hypothesis.score} is too low")
                continue
            ...
            update_success, _ = self.try_update_pose(
                result, detections.header, closest_object)
            if not update_success:
                self.outstanding_detections.append(OutstandingDetectionInfo(
                    result, detections.header, closest_object))
    ...
    def update_outstanding_detections(self):
        ...
        for outstanding in self.outstanding_detections:
            elapsed_nanoseconds = current_time.nanoseconds - \
                (outstanding.det_header.stamp.sec * 1e9) - \
                outstanding.det_header.stamp.nanosec
            elapsed_seconds = elapsed_nanoseconds / float(1e9)

            update_success, error_msg = self.try_update_pose(
                outstanding.det_result, outstanding.det_header, outstanding.closest_object)
    ...
    def try_update_pose(self, result: ObjectHypothesisWithPose, detection_header: Header, closest_object: str):
        ...
        parent: str = str(self.get_parameter(
            "init_data.{}.parent".format(result.hypothesis.class_id)).value)
        ...
        object_location.add_pose(
            trans_pose.pose, update_position, update_orientation)
        ...
        if (child == self.target_object or (self.target_object == "" and child == closest_object)) \
           and parent == "map":
            ...
            offset_pose.position.x = trans_pose.pose.position.x - \
                float(self.get_parameter(
                    "init_data.{}.pose.x".format(child)).value)
            offset_pose.position.y = trans_pose.pose.position.y - \
                float(self.get_parameter(
                    "init_data.{}.pose.y".format(child)).value)
            offset_pose.position.z = trans_pose.pose.position.z - \
                float(self.get_parameter(
                    "init_data.{}.pose.z".format(child)).value)
    ...
    def closest_object(self, detections: Detection3DArray) -> str:
        ...
        for detection in detections.detections:
            for result in detection.results:
                ...
                dist = math.sqrt(pose.position.x**2 +
                                 pose.position.y**2 + pose.position.z**2)
    ...
    def publish_pose(self):
        ...
        offset_transform.transform.translation = Vector3(
            x=offset_pose.pose.position.x, y=offset_pose.pose.position.y, z=offset_pose.pose.position.z)
        ...
        for object in self.objects.keys():
            parent = str(self.get_parameter(
                "init_data.{}.parent".format(object)).value)
            pose = PoseWithCovarianceStamped()

            pose.pose = cast(
                Location, self.objects[object]["location"]).get_pose()
            ...
            transform.transform.translation = Vector3(
                x=pose.pose.pose.position.x, y=pose.pose.pose.position.y, z=pose.pose.pose.position.z)
            ...
            if parent == "map":
                ...
                # need to assign individual components because a vector3 is not a point
                ...
                transform.header.frame_id = str(self.get_parameter(
                    "init_data.{}.parent".format(object)).value)
